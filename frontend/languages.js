// [languages.js] 翻译表(大市唱全产品共用): L() 查 g_language 表翻译(仅 zh/en 两列),
//   F/P/A/R() 均为恒等函数 —— 无字符串混淆。g_language_fix 为免翻译词表
...
//html += "<span style='display:inline-block;text-align:center;' onmousedown='" + A(() => { 添加鼠标按下(event.target); }) + "'>\r\n";
//html += "<span style='display:inline-block;text-align:center;' onmousedown='" + A("添加鼠标按下(event.target);") + "'>\r\n";
function A(action_js)//js代码中的字符串，仍然是js, A for action
{
    if (typeof action_js == "function")
    {
        let string;
        string = x.toString();
        let match;
        match = string.match(/\s*\(\s*\)\s*=>\s*\{(.*?)\}\s*/);
        if (!match)
            console.assert(0);
        return match[1];
    }
    ...
    {
        console.assert(typeof action_js == "string");
        return action_js;
    }
}
function P(path)//js代码中的字符串，是路径
{
    return path;
}
function R(regular)//正则表达式 !!代码还没有实现!!
{
    return regular;
}
